						...
						<div class="card">
							...
							<div class="card-body">
								<div class="horizontal-form-elements">
									<form class="form-horizontal" action="{{ route('film.update', [$d->id]) }}" method="POST" enctype="multipart/form-data">
										@csrf
										@method('PUT')
										<div class="row">
											<div class="col-lg-12">
												<div class="form-group @error('nama') has-error @enderror">
													...
													<div class="col-sm-6">
														<input required="required" name="nama" id="nama" type="text" class="form-control" placeholder="Nama" value="{{ old('nama') ? old('nama') : $d->nama }}">
														@error('nama')
												...
												<div class="form-group @error('author') has-error @enderror">
													...
													<div class="col-sm-6">
														<input required="required" name="author" id="author" type="text" class="form-control" placeholder="Author" value="{{ old('author') ? old('author') : $d->author }}">
														@error('author')
														<span class="help-block">{{ $message }}</span>
														@enderror
													</div>
												...
												<div class="form-group @error('tahun') has-error @enderror">
													...
													<div class="col-sm-6">
														<input required="required" name="tahun" id="tahun" type="number" class="form-control" placeholder="Tahun" min="1800" value="{{ old('tahun') ? old('tahun') : $d->tahun }}">
														@error('tahun')
														<span class="help-block">{{ $message }}</span>
														@enderror
													</div>
												...
												<div class="form-group @error('gambar') has-error @enderror">
													...
													<div class="col-sm-6">
														<input name="gambar" id="gambar" type="file" accept="image/*" class="form-control">
														@error('gambar')
														<span class="help-block">{{ $message }}</span>
														@enderror
													</div>
												...
												<div class="form-group @error('deskripsi') has-error @enderror">
													...
													<div class="col-sm-6">
														<textarea required="required" name="deskripsi" id="deskripsi" class="form-control">{{ old('deskripsi') ? old('deskripsi') : $d->deskripsi }}</textarea>
														@error('deskripsi')
														<span class="help-block">{{ $message }}</span>
														@enderror
													</div>
												...
												<div class="form-group @error('kategori') has-error @enderror">
													...
													<div class="col-sm-6">
														<select class="form-control" name="kategori" id="kategori">
															@php
															$selected = old('kategori_film_id') ? old('kategori_film_id') : $d->kategori_film_id;
															@endphp
															@foreach ($kategori as $k)
															<option @if($selected == $k->id) selected="selected" @endif value="{{ $k->id }}">{{ $k->nama }}</option>
															@endforeach
														</select>
														@error('kategori')
														<span class="help-block">{{ $message }}</span>
